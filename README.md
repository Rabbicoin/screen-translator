...
**Текст перевод**, **выбор языка перевода**, **ползунок крупности**, уголок **◢**
для размера окна и **✕**.
Текущий режим подсвечен, кнопки оригинала и картинки появляются, только когда
им есть что показать. Окно можно таскать мышью за любое свободное место.
...
видеть, куда писать, даже если сайт лежит или ещё не поднят. Ссылка одна на всю
строку — поэтому в тексте называются оба адреса, а щелчок ведёт в телеграм, как
в более быстрый способ связи. Поменять местами — переписать `footer_url`.
Очистить `footer_text` — строки не будет вовсе.
