...
Both shortcuts can be changed from the tray menu — click the one you want and
press your own keys.
...
**Клавиши можно поменять на свои** — трей → **Горячие клавиши**: щёлкните по
сочетанию и нажмите нужное. Программа тут же скажет, если оно занято другой
программой, и подхватит новое без перезапуска. Два условия: в сочетании должен
быть `Ctrl`, `Alt` или `Win` — одиночная клавиша, занятая глобально, перестала бы
работать во всех остальных программах (исключение — `F13`…`F24`, про них ниже);
и `Esc` назначить нельзя — ею закрывается окно перевода. Клавиша запоминается по коду, а не по букве, поэтому на русской
раскладке сочетание работает так же.

Зачем это может понадобиться: `Ctrl+Alt+Z` занят в Photoshop и Premiere (там это
«шаг назад»), а на польской, немецкой и чешской раскладках `Ctrl+Alt` — это
`AltGr`, то есть часть набора символов.

**Отдельная кнопка на игровой клавиатуре.** Боковые `G1`…`G6` Windows обычными
клавишами не считает: что они шлют, решает программа самой клавиатуры. Отсюда два
пути. Первый — назначить в ней на G-кнопку сразу `Ctrl+Alt+Z`; в переводчике тогда
менять нечего, кнопка просто заработает. Второй — назначить на неё `F13`…`F24` и
выбрать эту клавишу здесь: **их можно ставить в одиночку**, без `Ctrl` и `Alt`, —
на обычной клавиатуре таких клавиш нет, отнимать не у кого.
...
| Параметр | Значение |
|---|---|
| `hotkey_capture` | горячая клавиша выделения, например `<ctrl>+<shift>+q`. Меняется и мышью: трей → **Горячие клавиши** |
| `hotkey_clipboard` | горячая клавиша перевода буфера. Меняется и мышью: трей → **Горячие клавиши** |
...
достаточно сохранить `config.json`, перезапускать программу не нужно. Горячие
клавиши, изменённые **через трей**, тоже встают сразу; перезапуск нужен только
тем, кто правит их прямо в файле.
...
- **Не реагирует на `Ctrl+Alt+Z`.** Сочетание занято другой программой — при
  запуске программа сама показывает об этом плашку. Поменяйте клавишу: трей →
  **Горячие клавиши**. Проверить, что программа вообще жива, можно кликом по
  иконке в трее: она открывает то же выделение области.
