...
Правый клик по иконке в трее — язык перевода, оформление, сервис перевода,
автозапуск, выход. Нужен интернет: сам перевод делает Google Translate.
Распознавание текста работает без интернета.

Если бесплатный Google начал отказывать (перевод не приходит) или хочется
точнее — в том же меню «Сервис перевода» выберите DeepL или Google Cloud.
Программа сама спросит ключ и покажет, где его взять. У DeepL есть бесплатный
ключ, его тоже можно. Без ключа всё работает как работало.
