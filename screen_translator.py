...
# Медиана высоты слова (в пикселях исходной картинки), при которой стоит
# перепроверить распознавание без увеличения. Увеличение задумано ради мелкого
# экранного шрифта, но крупному тексту оно вредит: на снимке таблицы размеров
# после двукратного апскейла Tesseract терял все числа и оставлял один столбец,
# на сканах инструкций пропадали целые строки таблиц. Порог с запасом ниже
# замеренной границы вреда (там 25 px): лишняя проверка стоит доли секунды на
# исходном размере, а её итог принимается, только если он заметно лучше.
BIG_TEXT_HEIGHT = 20

# Слов прочитано так мало, что медиане верить нельзя: проверить второй масштаб
# дешевле, чем отдать человеку пустой перевод. Ровно так проваливалась таблица
# размеров в мессенджере — из тридцати шести слов после увеличения оставалось три.
TOO_FEW_WORDS = 12

# Какую долю своей площади слово второго прохода может делить с уже найденным,
# чтобы его ещё считали новым. Замерено: настоящая пропажа не пересекается с
# найденным вовсе (ровно 0.00), а повторное прочтение — от 0.24 и выше, причём
# до единицы дотягивает не всегда: в иероглифике Tesseract режет строку на куски
# по-разному на разных масштабах, и рамки съезжают на ползнака. Порог держим в
# этом зазоре, но не на нуле — новое слово вплотную к найденному задевает его
# рамку на несколько процентов.
MERGE_OVERLAP = 0.15

# Насколько увереннее должен быть претендент, чтобы занять уже занятое место.
# Обычно занявший прав и место не отдаёт. Но разбор макета «psm 3» иногда видит
# ячейку и читает её мусором: двухстрочную шапку таблицы размеров он прочёл как
# «eee» с уверенностью 21, и верное «Shoulder» с уверенностью 96 не пускалось —
# место занято. Отдавать место только при таком разрыве: у настоящих обрывков
# (кусок уже найденной строки) уверенность не ниже, чем у целой строки, и они
# по этому правилу не проходят.
SHIELD_CONF_GAP = 30

# Какую долю своей площади блок должен делить с большим соседом, чтобы считаться
# его обрывком и быть выброшенным. Замерено на демонстрационных снимках: у
# настоящих обрывков доля ровно 1.00 — они лежат внутри целиком, — а у разных
# надписей, чьи рамки просто задевают друг друга, не выше 0.75.
NESTED_BLOCK_SHARE = 0.9

# Сдвиг номера блока у строк, перенесённых из второго прохода. Номер блока
# служит признаком «та же колонка»: без сдвига строка из одного прохода склеилась
# бы в абзац со строкой другого только потому, что Tesseract дал им общий номер.
OTHER_PASS_BLOCK = 1000

# Площадь снимка, с которой стоит перепроверить разметку страницы. Разбор макета
# «psm 3» на сложной странице пропускает целые ячейки: на снимке таблицы размеров
# он не выдаёт шапку «Shoulder Width» и «Sleeve Length» ВООБЩЕ — даже с нулевой
# уверенностью, хотя тот же кусок отдельным выделением читается на 96. Замерено:
# прибавка появляется только на снимках около полутора мегапикселей, а на 0.36 и
# меньше дополнительный проход не находит ничего. Порог посередине, ближе к
# верхней границе: сложный макет требует места, а мелкое выделение интерфейса —
# главный сценарий программы — платить за это не должно.
COMPLEX_LAYOUT_PIXELS = 1_000_000

# Разметка для перепроверки. «psm 4» (одна колонка текста разного размера) видит
# двухстрочные ячейки шапки, которые пропускает «psm 3». Заменить им основной
# режим нельзя: он склеивает подпись на схеме с заголовком в одну строку, а
# «psm 11» (разреженный текст) и вовсе рассыпает связный текст — игровой диалог
...
# масштабами — без увеличения находятся строки таблицы, зато пропадает пункт 2 и
# теряется отрицание, «请勿在潮湿环境中使用» превращается в «请在潮湿环境中使用»,
# то есть запрет становится указанием. Там перевес всего в десятую часть слов, и
# менять проход нельзя. А там, где увеличение действительно рушит разметку,
# перевес полуторный и больше. Порог посередине разделяет эти случаи с запасом.
NATIVE_SCALE_MARGIN = 1.25
...
def merge_missing(primary, other, ratio):
    """Добавляем в ведущий проход то, что он потерял целиком. Вернём число слов.

    Масштаб выбирается один на всю картинку, а содержимое бывает поделено между
    масштабами. На скане инструкции увеличение даёт верное «请勿在潮湿环境中使用»
    и пункт 2, но теряет строки таблицы; без увеличения ровно наоборот. Выбором
    одного прохода такое не собрать — недостающее переносим из проигравшего.

    Переносим только то, что не накладывается на уже найденное: там, где оба
    прохода видят один и тот же текст, прочтение ведущего лучше — его для того и
    выбирали. «ratio» приводит координаты второго прохода к масштабу ведущего.

    Сверяемся по отдельным словам, а не по рамкам строк. Tesseract сплошь и
    рядом собирает в одну строку подпись у левого края и ячейку таблицы у
    правого: рамка такой строки тянется на всю ширину и почти вся пустая, так
    что по ней «новыми» выходят даже те куски, что давно найдены. На снимке
...
def _ocr_lines(img):
    ...
    def read(langs, mode=None):
        ...
        try:
            data = pytesseract.image_to_data(prepared, lang=langs,
                                             config=f"--oem 3 --psm {mode or psm}",
    ...
    # один столбец. Пропажа тянет за собой и разметку перевода — надписи
    # наползают друг на друга, потому что защита от наползания сверяется со
    # списком найденных блоков, а пропавшего блока в нём нет. Поэтому там, где
    # текст и так крупный, перепроверяем на исходном размере. Ведущим остаётся
    # тот проход, что прочитал больше, а из второго добираем блоки, которых у
    # ведущего нет вовсе. Мелкий шрифт до проверки не доходит — это главный
    # сценарий программы, и лишних полсекунды он не платит.
    if scale != 1.0:
        seen = sorted(w["y1"] - w["y0"] for words in lines.values() for w in words)
        median = seen[len(seen) // 2] / scale if seen else 0
        if median >= BIG_TEXT_HEIGHT or best_kept < TOO_FEW_WORDS:
            full, upscaled = scale, prepared
            prepared = preprocess(img, 1.0)
            native, kept, dropped = read(langs)
            log(f"медиана высоты слова {median:.0f} px, без увеличения "
                f"прочитано {kept}, отброшено {dropped}")
            ...
            log(f"склейка проходов: добавлено слов "
                f"{merge_missing(lines, native, ratio)}")

    # Разбор макета тоже теряет текст, причём молча: на снимке таблицы размеров
    # «psm 3» не выдаёт шапку «Shoulder Width» и «Sleeve Length» вообще — их нет
    # даже среди отброшенных по низкой уверенности. Тот же кусок, выделенный
    # отдельно, читается на 96: мешает не шрифт, а соседство фотографии и
    # таблицы на одной странице. Поэтому на крупном снимке проходим ещё раз
    # другой разметкой и добираем пропажи той же склейкой.
    if img.width * img.height >= COMPLEX_LAYOUT_PIXELS and psm != SECOND_LAYOUT_PSM:
...
def fix_pipe_as_i(seg):
    ...
    Цифру «1» в двойники не берём, хотя выглядит она похоже: в нумерованном
    списке это настоящий номер пункта, и подменять его буквой нельзя.
    """
    for i, word in enumerate(seg[:-1]):
        if word["text"].strip() in ("|", "l"):
...
def ocr_blocks(img):
    ...
    blocks = reading_order(drop_nested(blocks))
    log(f"OCR: строк {len(lines)}, блоков {len(blocks)}, отброшено мусора {skipped}")
    return join_blocks(blocks), blocks, langs


def drop_nested(blocks):
    """Убираем блок, целиком лежащий внутри другого, — это обрывок уже найденного.

    Второй проход разметки иногда возвращает кусок строки, которую основной
    прочитал целиком: «调节» внутри «可调节台灯安装说明», «silla» внутри целого
    испанского предложения. Склейка их пропускает — по отдельным словам место
    выглядит свободным, — а в готовых блоках такой обрывок ложится поверх
    большого, и перевод рисуется дважды, одно поверх другого.

    Условие нарочно строгое, «внутри почти целиком»: у соседних блоков рамки
    задевают друг друга на четверть и меньше, и терять их нельзя. Так, в списке
    файлов «12 мин. назад» перекрывает имя файла на три четверти — это разные
    надписи, и обе нужны.
    """
    out = []
    for block in blocks:
        ...
        обрывок = False
        for other in blocks:
            if other is block or len(other["text"]) <= len(block["text"]):
                continue
            ox0, oy0, ox1, oy1 = other["bbox"]
            dx = min(x1, ox1) - max(x0, ox0)
            dy = min(y1, oy1) - max(y0, oy0)
            if dx > 0 and dy > 0 and dx * dy >= NESTED_BLOCK_SHARE * area:
                обрывок = True
                break
        if not обрывок:
            out.append(block)
...
def render_overlay(img, blocks, translations, zoom=None):
    ...
    for idx, block in enumerate(blocks):
        ...
        for other_idx, other in enumerate(blocks):
            ...
            # Сосед снизу. Проверять «начинается ниже, чем кончаюсь я» нельзя:
            # рамки соседних строк заходят друг на друга на несколько пикселей
            # (выносные буквы плюс отступ в два пикселя), и признак молча
            # переставал срабатывать — блок разрастался на две своих высоты
            # прямо сквозь соседа. На таблице размеров в мессенджере из-за
            # этого налезали друг на друга все нижние примечания. Поэтому
