...
def _ocr_lines(img):
    ...
    # таблицы на одной странице. Поэтому проходим ещё раз другой разметкой и
    # добираем пропажи той же склейкой.
    #
    # Проход был ограничен снимками от мегапикселя — «сложный макет требует
    # места». Ограничение снято: оно давало обрыв качества на ровном месте. Одна
    # и та же таблица, снятая с разным масштабом, читалась то целиком, то без
